...
function handleTouchMove(event) {
    ...
    // Primäre Wischrichtung ermitteln
    const isVertical = Math.abs(deltaY) > Math.abs(deltaX);

    if (isVertical) {
        // Vertikales Wischen - horizontale Bewegung blockieren
        lightboxImg.style.transform = `translateY(${deltaY}px)`;
        return;
    }

    // Horizontales Wischen erlauben
...
function handleTouchEnd(event) {
    ...
    if (isVertical) {
        // Nur vertikales Wischen verarbeiten
        if (Math.abs(deltaY) > 100) closeLightbox();
        return;
    }
    ...
    if (Math.abs(deltaX) > 100) {
        ...
        // Sofortiger Bildwechsel ohne Animation
        changeImage(direction);
        
        // Position sofort zurücksetzen
        lightboxImg.style.transform = 'translateX(0)';
    } else {
        // Zurück zur Ausgangsposition ohne Animation
        lightboxImg.style.transition = 'none';
        lightboxImg.style.transform = 'translateX(0)';
        void lightboxImg.offsetWidth; // Reflow erzwingen
